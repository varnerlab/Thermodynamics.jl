...
function obj_function_temperature_pr(x, eos::PengRobinsonEquationOfState, P::Float64, V::Float64)
...
function compute_volume_pr(eos::PengRobinsonEquationOfState, P::Float64, T::Float64;  V::Float64 = 1.0)::Float64

    # setup the calculation -
    xinitial = [V]
    OF(p) = obj_function_volume_pr(p, eos, P, T)
...
function compute_temperature_pr(eos::PengRobinsonEquationOfState, P::Float64, V::Float64; T::Float64)::Float64

    # setup the calculation -
    xinitial = [T]
    OF(p) = obj_function_temperature_pr(p, eos, P, V)
...
# --- Lee Kesler EOS ----------------------------------------------------------------------------- #
function compute_volume_lk(eos::LeeKeslerEquationOfState, V::Float64, T::Float64)::Float64
...
function compute_pressure_lk(eos::LeeKeslerEquationOfState, V::Float64, T::Float64)::Float64
...
function compute_temperature_lk(eos::LeeKeslerEquationOfState, P::Float64, V::Float64)::Float64 
    return 0.0
end
# ------------------------------------------------------------------------- #
...
function pressure(eos::AbstractEquationOfState, V::Float64, T::Float64)::Float64
    ...
    elseif (typeof(eos) == LeeKeslerEquationOfState)
        return compute_pressure_lk(eos, V, T)
    else
...
function volume(eos::AbstractEquationOfState, P::Float64, T::Float64)::Float64
    ...
    elseif (typeof(eos) == LeeKeslerEquationOfState)
        return compute_volume_lk(eos, P, T)
    else
        throw(ArgumentError("$(typeof(eos)) is not supported"))
    end
end
...
function temperature(eos::AbstractEquationOfState,P::Float64,V::Float64)::Float64
    ...
    elseif (typeof(eos) == LeeKeslerEquationOfState)
        return compute_temperature_lk(eos, P, V)
    else
        throw(ArgumentError("$(typeof(eos)) is not supported"))
    end
end
